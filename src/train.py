...
def main(argv=None):
    t = 0
    #TIME_P = np.full(NUM_LABELS, 1 / 19)
    TIME_P = np.full(NUM_LABELS, 1 / 19)
    # Load the dataset.
        ...
        for mate_epoch in range(MATE_EPOCHS):
            ...
            for epoch_i in range(0, EPOCHS):
                ...
                for step, batch in enumerate(labeled_dataloader):
                    ...
                    # Perform a backward pass to calculate the gradients.这里的关键是，尽管 loss1 是由 modelf1 计算得到的，但它也被用于更新 modelg2 的梯度。这样做是为了利用 modelf1 的预测结果来指导 modelg2 的训练。
                    #loss1.sum().backward()
            ...
            probs = F.softmax(logits, dim=1) #317,19
            ...
            index_label_weights = [[] for _ in range(NUM_LABELS)]
            for i in range(len(labels)):
                index_label_weights[int(labels[i])].append(label_weights[i])
            sorted_index_label_weights = []

            for i in range(NUM_LABELS):
                sorted_indices = sorted(range(len(index_label_weights[i])), key=lambda x: index_label_weights[i][x], reverse=True)
                sorted_index_label_weights.append([index_label_weights[i][j] for j in sorted_indices])
            ...
            for i in range(NUM_LABELS):
                if(len(sorted_index_label_weights[i]) != 0): 
                    TIME_P[i] = TIME_P[i] * M + (1 - M) * torch.quantile(torch.tensor(sorted_index_label_weights[i]),0.8).to(device).item()
            
            print("TIME_P")
            print(TIME_P)
            sort = []
            for i in range(len(labels)):
                if(label_weights[i] > TIME_P[labels[i]]):
                    sort.append(i)

            print(str(len(sort) / len(labels)) + " data remained")
